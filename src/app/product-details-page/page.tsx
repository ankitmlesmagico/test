...
export default function ProductDetailsPage() {
  ...
  const step = ['Amazon'];
  const BaseUrl = 'https://api.epic.dev.esmagico.in';
  const id = 1;
  console.log(
    subCategoryOptions,
    'categoryOptions',
  ...
  const getCategoryOptions = async () => {
    const url = `${BaseUrl}/product-listing/category?category_id=${id}`;
    try {
  ...
  useEffect(() => {
    ...
    console.log('product details are here ', storedProduct);
    if (storedProduct !== null) {
      handleFetchProduct(JSON.parse(storedProduct).product_listing_id);
    }
    getCategoryOptions();
  }, []);

  const handleFetchProduct = async (productListingId: string) => {
    try {
      const response = await axios.get<any>(
        `${BaseUrl}/product-listing/${productListingId}`,
        {
          headers: {
  ...
  const updateProductListing = async (productId: any, productData: any) => {
    ...
    try {
      const response = await axios.patch(
        `https://api.epic.dev.esmagico.in/product-listing/${productId}`,
        {
          content: productData,
        },
        {
